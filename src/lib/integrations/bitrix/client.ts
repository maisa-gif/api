...
export class BitrixClient {
  ...
  /**
   * TEMPORARY debug helper — tries several phone-search strategies against
   * the real Bitrix portal and returns all raw results, to figure out
   * which one actually works instead of guessing. Delete along with the
   * diagnostic route that calls it once findContactsByPhone is fixed for
   * real.
   */
  async debugPhoneSearch(phone: string): Promise<Record<string, unknown>> {
    const digits = normalizePhone(phone);
    ...
    try {
      attempts.exactAsIs = await this.call("crm.contact.list", {
        filter: { PHONE: digits },
        select: ["ID", "NAME", "PHONE"],
      });
    } catch (err) {
      attempts.exactAsIs = { error: err instanceof Error ? err.message : String(err) };
    }

    try {
      attempts.exactWithCC = await this.call("crm.contact.list", {
        filter: { PHONE: withCC },
        select: ["ID", "NAME", "PHONE"],
      });
    } catch (err) {
      attempts.exactWithCC = { error: err instanceof Error ? err.message : String(err) };
    }

    try {
      attempts.exactWithoutCC = await this.call("crm.contact.list", {
        filter: { PHONE: withoutCC },
        select: ["ID", "NAME", "PHONE"],
      });
    } catch (err) {
      attempts.exactWithoutCC = { error: err instanceof Error ? err.message : String(err) };
    }

    try {
      attempts.percentPhoneLast8 = await this.call("crm.contact.list", {
        filter: { "%PHONE": last8 },
        select: ["ID", "NAME", "PHONE"],
      });
    } catch (err) {
      attempts.percentPhoneLast8 = { error: err instanceof Error ? err.message : String(err) };
    }

    try {
      attempts.duplicateFindByCommWithCC = await this.call("crm.duplicate.findbycomm", {
        entity_type: "CONTACT",
        type: "PHONE",
        values: [withCC],
      });
    } catch (err) {
      attempts.duplicateFindByCommWithCC = { error: err instanceof Error ? err.message : String(err) };
    }

    try {
      attempts.duplicateFindByCommWithoutCC = await this.call("crm.duplicate.findbycomm", {
        entity_type: "CONTACT",
        type: "PHONE",
        values: [withoutCC],
      });
    } catch (err) {
      attempts.duplicateFindByCommWithoutCC = { error: err instanceof Error ? err.message : String(err) };
    }

    return { digits, withCC, withoutCC, last8, attempts };
  }
