...
### Meeting transcripts sync (Drive → Bitrix24 + CNN)

When "Take notes with Gemini" is used on a Google Meet call, Google Drive gets a Doc named after the meeting title (== the patient's name, since the synced Calendar event is titled that way), e.g. "Victor Cirne Carvalho – 2026/07/21 15:51 GMT-03:00 – Anotações do Gemini". This pushes each new one onto the matching patient's Bitrix24 CRM timeline, and links it on the matching CNN appointment's observações:
