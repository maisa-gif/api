...
- `src/lib/sync/clinica-nas-nuvens-google-calendar.ts` — fetches upcoming CNN appointments and creates/updates matching Google Calendar events, tracked via the `SyncedAppointment` table so re-runs update instead of duplicating. CNN reschedules by deleting the old appointment and creating a new one under a different ID (not editing in place), so after syncing the current window this also deletes the Google event for any previously-synced appointment whose ID vanished from CNN but whose last known date was today or later — rows that simply aged out of the window (past dates) are left alone. Set `CLINICA_NAS_NUVENS_EXECUTOR_ID` to restrict the sync to one professional (CNN's `idPessoaExecutor`, found via `GET /executor-agenda/lista`) — leave unset to sync every professional's appointments.
- Every newly created event gets a Google Meet link attached automatically (`GoogleCalendarClient.insertEvent` requests `conferenceData` with `conferenceDataVersion=1`), so the doctor has a ready-to-join link on the appointment without adding one manually. Updates use `PATCH` instead of `PUT` specifically so re-syncing an existing appointment (e.g. time changed in CNN) doesn't wipe out its Meet link.
...
- After a successful attach, the matched contact's open deal in Bitrix's "Funil Comercial" pipeline (category id `5`) is moved to the "Avaliação Realizada" stage (`STATUS_ID` `C5:FINAL_INVOICE` — both hardcoded in `drive-transcripts-bitrix.ts`, found via a temporary diagnostic route against `crm.dealcategory.list`/`crm.dealcategory.stage.list`). This is best-effort: a missing/already-past-that-stage deal doesn't fail the sync, it just leaves a note in `SyncedTranscript.errorMessage`.
